...
-(void)clickFollowBtn:(UIButton *)btn {
    ...
    if ([model.uidStr isEqual:[Config getOwnID]]) {
        [MBProgressHUD showError:@"不能关注自己"];
        btn.enabled = YES;
        return;
    }
...
-(void)creatNavi {
    ...
    NSArray *sgArr1 = [NSArray arrayWithObjects:@"收益榜",@"贡献榜", nil];
    ...
    NSArray *sgArr2 = [NSArray arrayWithObjects:@"日榜",@"周榜",@"月榜",@"总榜", nil];
