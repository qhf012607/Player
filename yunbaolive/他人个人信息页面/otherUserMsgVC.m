...
- (void)creatBottomView{
    ...
    NSArray *bottomTitleArr = @[@"关注",@"私信",@"拉黑"];
        ...
        [YBToolClass postNetworkWithUrl:@"User.setAttent" andParameter:@{@"touid":_userID} success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
            if (code == 0) {
                ...
                if ([isattent isEqual:@"1"]) {
                    [attionBtn setTitle:@"已关注" forState:UIControlStateNormal];
                    NSLog(@"关注成功");
                    if (self.block) {
                        self.block();
                    }
                ...
                {
                    [attionBtn setTitle:@"关注" forState:UIControlStateNormal];
                    NSLog(@"取消关注成功");
                }
            }
        } fail:^{
...
//                if ([attionBtn.titleLabel.text isEqual:@"关注"]) {
...
//            }else{
//                [MBProgressHUD showError:@"对方未注册私信"];
//            }
//        }];
    ...
    if (sender == blackBtn) {
        [YBToolClass postNetworkWithUrl:@"User.setBlack" andParameter:@{@"touid":_userID} success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
            if (code == 0) {
                if ([blackBtn.titleLabel.text isEqual:@"拉黑"]) {
                    [blackBtn setTitle:@"已拉黑" forState:UIControlStateNormal];
                    [MBProgressHUD showError:@"已拉黑"];
                }
                else{
                    [blackBtn setTitle:@"拉黑" forState:UIControlStateNormal];
                    [MBProgressHUD showError:@"已解除拉黑"];
                }
...
- (void)creatCollectionHeader:(NSDictionary *)dic{
    ...
    if ([laingname isEqual:@"0"]) {
        ...
        IDLabel.text = [NSString stringWithFormat:@"靓:%@",laingname];
    }
    ...
    [fansBtn setTitle:[NSString stringWithFormat:@"%@  粉丝",minstr([dic valueForKey:@"fans"])] forState:0];
    ...
    [followBtn setTitle:[NSString stringWithFormat:@"%@  关注",minstr([dic valueForKey:@"follows"])] forState:0];
    ...
    [addImpBtn setTitle:@"+添加印象" forState:0];
    ...
    NSArray *rankTitleArr = @[[NSString stringWithFormat:@"%@贡献榜",[common name_votes]],@"守护榜"];
