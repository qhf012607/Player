...
- (void)doFollow{
    ...
    [YBToolClass postNetworkWithUrl:postUrl andParameter:postDic success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        if (code == 0) {
            [MBProgressHUD showError:@"关注对方成功"];
            if (self.block) {
                self.block(1);
            }
...
- (void)photoClick {
  ...
  [lib enumerateGroupsWithTypes:ALAssetsGroupSavedPhotos usingBlock:^(ALAssetsGroup *group, BOOL *stop) {
    ...
    [self presentViewController:photoPickerVC animated:YES completion:NULL];
      ...
      UIAlertController *alertC = [UIAlertController alertControllerWithTitle:@"没有相册权限" message:@"请到设置页面获取相册权限" preferredStyle:UIAlertControllerStyleAlert];
...
//                }
//            }else {
////                [MBProgressHUD showError:@"对方暂时拒绝接收您的消息"];
//                [MBProgressHUD showError:@"对方暂时拒绝接收您的消息"];
//            }
//        }else {
////            [MBProgressHUD showError:msg];
...
//                [MBProgressHUD showError:@"取消拉黑成功"];
                [MBProgressHUD showError:@"取消拉黑成功"];
            }
        }else {
//            [MBProgressHUD showError:msg];
...
//    [self getGroupMemberListWithGetMessageFlag:YES];
    ...
    _emojiV.delegate = self;
    [[UIApplication sharedApplication].keyWindow addSubview:_emojiV];
}
