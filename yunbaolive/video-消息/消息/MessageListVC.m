...
//                }else{
//                    [MBProgressHUD showError:[NSString stringWithFormat:@"%@未加入IM",error.localizedDescription]];
//                    [MBProgressHUD showError:[NSString stringWithFormat:@"%@未加入IM",error.localizedDescription]];
//                }
//            }];
        }];
...
-(void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        [MBProgressHUD showError:@"系统消息无法删除"];
        return;
    }
...
//    }
    if (_unreadCount>0) {
        ...
        UIAlertView *alerts = [[UIAlertView alloc]initWithTitle:@"暂无未读消息" message:nil delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
        [alerts show];
    }
}
