...
- (void)creatUI:(NSDictionary *)subDic{
    ...
    [buyBtn setTitle:@"立即开通" forState:0];
...
- (void)btnClick:(grardButton *)sender{
    ...
    NSArray *firstArr = [[infoDic valueForKey:@"list"][selectIndex] valueForKey:@"privilege"];
    for (int i = 0; i < privilege.count; i ++) {
        NSDictionary *itemDic = privilege[i];
...
//    }else{
        if ([_guardType isEqual:@"1"] && selectIndex == 2) {
            gAlert = [[guardAlertView alloc]initWithFrame:CGRectMake(0, 0, _window_width, _window_height) andType:1 andMsg:@"您当前为主播的月守护，开通年守护将覆盖您的月守护剩余时长，是否开通？"];
            ...
            gAlert = [[guardAlertView alloc]initWithFrame:CGRectMake(0, 0, _window_width, _window_height) andType:0 andMsg:@"您当前为主播的年守护\n无法开通7天/月守护"];
            __weak shouhuView *wSelf = self;
            gAlert.block = ^(BOOL isSure) {
                if (isSure) {
                    [wSelf goBuy];
            ...
            switch (selectIndex) {
                case 0:
                    msg = [NSString stringWithFormat:@"您将花费%@%@，为主播开通7天守护",minstr([[infoDic valueForKey:@"list"][selectIndex] valueForKey:@"coin"]),[common name_coin]];
                    break;
                case 1:
                    msg = [NSString stringWithFormat:@"您将花费%@%@，为主播开通月守护",minstr([[infoDic valueForKey:@"list"][selectIndex] valueForKey:@"coin"]),[common name_coin]];
                    break;
                case 2:
                    msg = [NSString stringWithFormat:@"您将花费%@%@，为主播开通年守护",minstr([[infoDic valueForKey:@"list"][selectIndex] valueForKey:@"coin"]),[common name_coin]];
                    break;
...
- (void)requestData{
    ...
    NSString *url = [purl stringByAppendingFormat:@"/?service=Guard.GetList"];
    ...
    [session POST:url parameters:parameterDic
         progress:nil success:^(NSURLSessionDataTask *task, id responseObject) {
             ...
             if([number isEqualToNumber:[NSNumber numberWithInt:200]])
             {
                 ...
                 {
                     infoDic = [[data valueForKey:@"info"] firstObject];
