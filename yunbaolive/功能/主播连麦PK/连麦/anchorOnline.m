...
- (void)creatUI{
    ...
    _searchT.placeholder = @"输入您要搜索的主播昵称或ID";
...
- (void)startPK:(anchorModel *)model{
    [YBToolClass postNetworkWithUrl:@"Livepk.CheckLive" andParameter:@{@"stream":minstr(model.stream)} success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        if (code == 0) {
            [self.delegate startLink:[anchorModel dicWithModel:model]];
            [self.delegate removeShouhuView];
        }else{
            [MBProgressHUD showError:msg];
        }
