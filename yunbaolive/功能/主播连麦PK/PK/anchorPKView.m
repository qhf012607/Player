...
- (void)creatUI{
    ...
    timeL.text = [NSString stringWithFormat:@"PK时间 %@",[self seconds:timeCount]];
...
- (void)changeTimeL{
    ...
    timeL.text = [NSString stringWithFormat:@"PK时间 %@",[self seconds:timeCount]];
    if (timeCount <= 0) {
        timeL.text = [NSString stringWithFormat:@"PK时间 00:00"];
        [timer invalidate];
        timer = nil;
    }
...
- (void)changeChengfaTimeL{
    timeCount --;
    timeL.text = [NSString stringWithFormat:@"惩罚时间 %@",[self seconds:timeCount]];
    if (timeCount <= 0) {
        timeL.text = [NSString stringWithFormat:@"惩罚时间 00:00"];
        [timer invalidate];
        timer = nil;
