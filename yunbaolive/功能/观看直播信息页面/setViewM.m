...
-(void)leftviews{
    ...
    [_newattention setTitle:@"关注" forState:UIControlStateNormal];
    ...
    if ([liangname isEqual:@"0"]) {
        ...
        _onlineLabel.text = [NSString stringWithFormat:@"靓:%@",liangname];
    }
    //魅力值//魅力值
    //修改 魅力值 适应字体 欣
    ...
    [_guardBtn setTitle:@"守护 虚位以待>" forState:0];
    _guardBtn.layer.cornerRadius = 10;
    _guardBtn.layer.masksToBounds = YES;
    guardWidth = [[YBToolClass sharedInstance] widthOfString:@"守护 虚位以待>" andFont:[UIFont systemFontOfSize:12] andHeight:20];
...
-(void)changeGuardButtonFrame:(NSString *)nums{
    [_guardBtn setTitle:[NSString stringWithFormat:@"守护 %@人 >",nums] forState:0];
