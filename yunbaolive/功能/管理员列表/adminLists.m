...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    label.text = [NSString stringWithFormat:@"    当前管理员（%ld/%@）",self.allArray.count,self.addAdmins];
    return label;
}
...
- (void)delateAdminUser:(fansModel *)model{
    ...
    UIAlertController *alertContro = [UIAlertController alertControllerWithTitle:@"温馨提示" message:[NSString stringWithFormat:@"是否确定取消%@的管理员身份？",model.name] preferredStyle:UIAlertControllerStyleAlert];
    UIAlertAction *cancleAction = [UIAlertAction actionWithTitle:@"不了" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
    }];
