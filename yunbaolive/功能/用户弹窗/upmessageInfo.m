...
-(void)upMessage{
    ...
    NSArray *array = @[@"关注",[NSString stringWithFormat:@"送出%@",[common name_coin]],@"粉丝",[NSString stringWithFormat:@"收入%@",[common name_votes]]];
    ...
    [YBToolClass postNetworkWithUrl:@"Live.getPop" andParameter:getPop success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        if (code == 0) {
            ...
            if ([minstr([singleUserArray valueForKey:@"consumption"]) intValue] > 10000) {
                NSString *attString = [NSString stringWithFormat:@"%.1f 万",[minstr([singleUserArray valueForKey:@"consumption"]) intValue]/10000.0];
            ...
            if ([minstr([singleUserArray valueForKey:@"fans"]) intValue] > 10000) {
                NSString *attString = [NSString stringWithFormat:@"%.1f 万",[minstr([singleUserArray valueForKey:@"fans"]) intValue]/10000.0];
            ...
            if ([minstr([singleUserArray valueForKey:@"votestotal"]) intValue] > 10000) {
                NSString *attString = [NSString stringWithFormat:@"%.1f 万",[minstr([singleUserArray valueForKey:@"votestotal"]) intValue]/10000.0];
            ...
            if ([minstr([singleUserArray valueForKey:@"follows"]) intValue] > 10000) {
                NSString *attString = [NSString stringWithFormat:@"%.1f 万",[minstr([singleUserArray valueForKey:@"follows"]) intValue]/10000.0];
...
{
    if (!_forceBtn) {
        _forceBtn = [self YBBottomButton:_forceBtn title:@"关注" titleFont:13];
...
{
    if (!_messageBtn) {
        _messageBtn = [self YBBottomButton:_messageBtn title:@"私信" titleFont:13];
