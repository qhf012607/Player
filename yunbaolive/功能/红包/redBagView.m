...
//        }
    ...
    [sendBtn setTitle:[NSString stringWithFormat:@"发红包 100%@",[common name_coin]] forState:0];
...
- (void)sendBtnClick{
    if (coinT.text == NULL || coinT.text == nil || coinT.text.length == 0) {
        [MBProgressHUD showError:@"请输入红包金额"];
        return;
    }
    if (numT.text == NULL || numT.text == nil || numT.text.length == 0) {
        [MBProgressHUD showError:@"请输入红包数量"];
        return;
    }
        ...
        if ([coinT.text intValue] >= 10000) {
            if (([coinT.text intValue] * [numT.text intValue]) % 1000 == 0) {
                if (([coinT.text intValue] * [numT.text intValue]) % 10000 == 0) {
                    coinStr = [NSString stringWithFormat:@"发红包 %dw%@",[coinT.text intValue]/10000,[common name_coin]];
                }else{
                    coinStr = [NSString stringWithFormat:@"发红包 %.1fw%@",[coinT.text intValue]/10000.0,[common name_coin]];
                }
            }else{
                coinStr = [NSString stringWithFormat:@"发红包 %.2fw%@",[coinT.text intValue] /10000.00,[common name_coin]];
            }
        }else{
            coinStr = [NSString stringWithFormat:@"发红包 %d%@",[coinT.text intValue],[common name_coin]];
        }
        ...
        if ([coinT.text longLongValue] * [numT.text longLongValue] >= 10000) {
            if (([coinT.text longLongValue] * [numT.text longLongValue]) % 1000 == 0) {
                if (([coinT.text longLongValue] * [numT.text longLongValue]) % 10000 == 0) {
                    coinStr = [NSString stringWithFormat:@"发红包 %lldw%@",([coinT.text longLongValue] * [numT.text longLongValue])/10000,[common name_coin]];
                }else{
                    coinStr = [NSString stringWithFormat:@"发红包 %.1fw%@",([coinT.text longLongValue] * [numT.text longLongValue])/10000.0,[common name_coin]];
                }
            }else{
                coinStr = [NSString stringWithFormat:@"发红包 %.2fw%@",([coinT.text longLongValue] * [numT.text longLongValue])/10000.00,[common name_coin]];
            }
        }else{
            coinStr = [NSString stringWithFormat:@"发红包 %d%@",[coinT.text intValue] * [numT.text intValue],[common name_coin]];
        }
    }
