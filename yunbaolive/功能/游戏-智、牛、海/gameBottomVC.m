...
-(void)cancle{
    ...
    if ([games isEqual:@"1"] ) {
        [MBProgressHUD showError:@"请等待游戏结束"];
        return;
    }
...
-(instancetype)initWIthDic:(NSDictionary *)dic andIsHost:(BOOL)ok andMethod:(NSString *)method andMsgtype:(NSString *)msgtype{
    ...
    if (self) {
        ...
        if ([_method isEqual:@"startGame"]) {
            ...
            _personname = @[@"大乔",@"貂蝉",@"小乔"];
            _personnameX =@[@"大乔X2",@"貂蝉X2",@"小乔X2"];
        ...
        if([_method isEqual:@"startCattleGame"]) {
            ...
            _personname = @[@"星星",@"球球",@"仔仔"];
            _personnameX =@[@"星星X2",@"球球X2",@"仔仔X2"];
        ...
        if ([_method isEqual:@"startLodumaniGame"]) {
            ...
            _personname   = @[@"罗伯茨",@"平局",@"基德"];
            _personnameX = @[@"罗伯茨X2",@"平局X2",@"基德X2"];
...
-(void)setView{
    ...
    if (!imageVS) {
        ...
        [imageVS setTitle:@"即将开始，请稍后" forState:UIControlStateNormal];
    ...
    [YBToolClass postNetworkWithUrl:url andParameter:nil success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        if (code == 0) {
            ...
            if ([keyArray containsObject:@"isshow"]) {
                ...
                if (![isshow isEqual:@"0"]) {
                    UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"" message:[NSString stringWithFormat:@"您的余额不足%@将被下庄",[gameState getzhuanglimit]] delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
                    [alert show];
                }
            }
            //刷新我自己的余额
            ...
            if (shangzhuang) {
                ...
                zhuangresult.banker_profit.text = [NSString stringWithFormat:@"庄家 %@",banker_profit];
                zhuangresult.gamecoin.text = [NSString stringWithFormat:@"本家 %@",gamecoin];
                //显示动画s
                ...
                if (succcess >0) {
                    ...
                    UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"赢了哦" message:money delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
                    [alert show];
                }
                else {
                    //没中奖
...
-(void)kaishiapai{
    if (imageVS) {
        [imageVS setTitle:@"开始支持" forState:UIControlStateNormal];
...
-(void)kaishigetresult{
    if (imageVS) {
        [imageVS setTitle:@"揭晓结果" forState:UIControlStateNormal];
        imageVS.transform = CGAffineTransformMakeScale(0.01f, 0.01f);
        [UIView beginAnimations:nil context:UIGraphicsGetCurrentContext()];
        [UIView setAnimationCurve:UIViewAnimationCurveEaseOut];
        [UIView setAnimationDuration:0.5f];
...
-(void)stakePokers:(UIButton *)sender{
    ...
    if ([_bankerid isEqual:[Config getOwnID]]) {
        [MBProgressHUD showError:@"庄家无法下注"];
        return;
    }
    if ([_money isEqual:@"0"]) {
