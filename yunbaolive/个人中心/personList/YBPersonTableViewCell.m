...
#import "YBToolButton.h"
@interface YBPersonTableViewCell ()<YBBottpmViewDelegate,PersonBottomViewDelegate>
{
    //名字宽度
...
-(void)perform:(NSString *)text{
    if ([text rangeOfString:@"观看历史"].location != NSNotFound) {
    
        [self.personCellDelegate pushLiveNodeList];
        
    }else if ([text rangeOfString:@"电视投屏"].location != NSNotFound){
        
        [self.personCellDelegate pushFansList];
        
    }else if ([text rangeOfString:@"视频收藏"].location != NSNotFound){
...
//    }];
    ...
    if ([laingname isEqual:@"0"]) {
        ...
        _IDL.text = [NSString stringWithFormat:@"靓:%@",laingname];
    }
...
//    [_level_anchorView setImage:[UIImage imageNamed:[NSString stringWithFormat:@"host_%@",_model.level_anchor]]];
    [_iconView sd_setImageWithURL:[NSURL URLWithString:_model.avatar] placeholderImage:[UIImage imageNamed:@"头像"]];
...
//    backImgView.backgroundColor = normalColors;
    backImgView.image = [UIImage imageNamed:@"头像背景"];
...
//    iconView.frame = CGRectMake(0,0,80,80);
    ...
    nameLabel.textColor = RGB_COLOR(@"#FFFFFF",1 );
...
//    self.editBtn2 = editBtn2;
    ...
    ID.textColor = RGB_COLOR(@"#FFFFFF",1 );
...
//    bottomView.BottpmViewDelegate = self;
    PersonBottomView *bottomView = [[PersonBottomView alloc]init];
    bottomView.BottpmViewDelegate = self;
    ...
    YBToolButton * historyBtn = [[YBToolButton alloc] initWithFrame:CGRectMake(0, 0, self.bottomView.frame.size.width / 3, self.bottomView.frame.size.height)];
    [historyBtn setTitle:@"观看历史" forState:UIControlStateNormal];
    [self.bottomView addSubview:historyBtn];
...
//    _IDL.text = [NSString stringWithFormat:@"ID:121212121212"];
...
//        make.top.mas_equalTo(self.nameLabel.mas_bottom).offset(10);
    ...
    [self.sexView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.IDL.mas_right).offset(3);
