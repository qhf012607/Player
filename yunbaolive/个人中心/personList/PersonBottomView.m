...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame])
    {
        ...
        NSArray *arr= @[@0,@0,@0];
        self.numArr = arr;
        NSArray *nameArr = @[@"直播",@"关注",@"粉丝"];
        self.btnArr = nameArr;
        NSArray *ImageArr = @[@"lishi",@"shoucang(1)",@"touying"];
...
-(void)setUI
{
    ...
    self.liveBtn =  [self YBBottomButton:self.liveBtn title:@"观看历史" titleFont:16  numValue:_numArr[0] IndexBtn:0];
    self.forceBtn = [self YBBottomButton:self.forceBtn title:@"视频收藏" titleFont:16 numValue:_numArr[1] IndexBtn:1];
    self.fanBtn =   [self YBBottomButton:self.forceBtn title:@"电视投屏" titleFont:16 numValue:_numArr[2] IndexBtn:2];
...
-(void)layoutSubviews
{
    ...
    [self.liveBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.width.mas_equalTo(self.frame.size.width/self.btnArr.count - 1);
    }];
    [self.forceBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.liveBtn.mas_right).offset(1);
        make.top.width.height.equalTo(self.liveBtn);
    }];
    [self.fanBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.forceBtn.mas_right).offset(1);
        make.top.width.height.equalTo(self.liveBtn);
    }];
...
- (UIButton *)YBBottomButton:(UIButton *)button  title:(NSString *)title titleFont:(CGFloat)titleFont numValue:(NSString *)numValue IndexBtn:(NSInteger)indexBtn
{
    button = [[YBToolButton alloc]init];
    ...
    [button setImage:[UIImage imageNamed:self.imgArr[indexBtn]] forState:UIControlStateNormal];
