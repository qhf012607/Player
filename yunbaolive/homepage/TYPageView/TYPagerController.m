...
{
    return CGRectMake(index * CGRectGetWidth(frame), 80 * Rate, CGRectGetWidth(frame), CGRectGetHeight(frame));
}
...
- (void)moveToControllerAtIndex:(NSInteger)index animated:(BOOL)animated
{
    ...
    [_contentView setContentOffset:CGPointMake(index * _window_width,0) animated:NO];
...
//    CGFloat contentTopEdging = _contentTopEdging + [self statusBarHeight] + statusbarHeight;
    ...
    _contentView.frame = CGRectMake(0, 0, _window_width, _window_height);
    _contentView.contentSize = CGSizeMake(_countOfControllers * _window_width,0);
    _contentView.contentOffset = CGPointMake(_window_width*_curIndex, 0);
}
