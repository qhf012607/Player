...
- (void)setnodejszhuboDic:(NSDictionary *)zhubodic Handler:(getResults)handler andlivetype:(NSString *)livetypes{
    ...
    self.playDoc = zhubodic;
     justonce = 0;
    AFHTTPSessionManager *session = [AFHTTPSessionManager manager];
    NSString *url = [purl stringByAppendingFormat:@"?service=Live.enterRoom&uid=%@&token=%@&liveuid=%@&city=%@&stream=%@",[Config getOwnID],[Config getOwnToken],zhubodic[@"uid"],[cityDefault getMyCity],zhubodic[@"stream"]];
...
-(void)addNodeListen:(NSDictionary *)dic
{
    ...
    self.ChatSocket = [[SocketIOClient alloc] initWithSocketURL:url config:@{@"log": @NO,@"forcePolling":@YES}];
    ...
    [self.ChatSocket on:@"broadcastingListen" callback:^(NSArray* data, SocketAckEmitter* ack) {
        justonce= 1;
...
-(void)getmessage:(NSDictionary *)msg andMethod:(NSString *)method{
    ...
    if ([method isEqual:@"requestFans"]) {
        ...
        if ([[data valueForKey:@"code"]isEqualToNumber:@0]) {
            ...
            if([self.socketDelegate respondsToSelector:@selector(addZombieByArray:)]) {
    ...
    if ([method isEqual:@"SendMsg"]) {
        ...
        if([msgtype isEqual:@"2"])
        {
            ...
            if ([sub_heart containsObject:@"heart"]) {
                ...
                if([self.socketDelegate respondsToSelector:@selector(light:)]) {
                ...
                NSString *guardType =minstr([msg valueForKey:@"guard_type"]);
                NSString *uhead = minstr([msg valueForKey:@"uhead"]);
                NSDictionary *chat = [NSDictionary dictionaryWithObjectsAndKeys:uname,@"userName",ct,@"contentChat",levell,@"levelI",ID,@"id",titleColor,@"titleColor",vip_type,@"vip_type",liangname,@"liangname",usertype,@"usertype",isAnchor,@"isAnchor",guardType,@"guard_type",uhead,@"uhead",nil];
                if([self.socketDelegate respondsToSelector:@selector(messageListen:)]) {
        ...
        if([msgtype isEqual:@"0"])
        {
            ...
            if ([action isEqual:@"1"]) {
                NSLog(@"用户离开，%@",msg);
                if([self.socketDelegate respondsToSelector:@selector(UserLeave:)]) {
            ...
            //用户进入
            if ([action isEqual:@"0"]) {
                if([self.socketDelegate respondsToSelector:@selector(UserAccess:)]) {
        ...
        if ([msgtype isEqual:@"1"]) {
            ...
            if ([action isEqual:@"18"]) {
                NSLog(@"直播关闭");
                if([self.socketDelegate respondsToSelector:@selector(LiveOff)]) {
        ...
        if([msgtype isEqual:@"255"])
        {
            ...
            //
            if ([action isEqual:@"0"]) {
                if ([self.socketDelegate respondsToSelector:@selector(showRedbag:)]){
    ...
    //增加映票
        ...
        if ([msgtype isEqual:@"26"])
        {
            ...
            if (![uid isEqual:[Config getOwnID]]) {
                if ([self.socketDelegate respondsToSelector:@selector(addvotesdelegate:)]){
    ...
    {
        if ([self.socketDelegate respondsToSelector:@selector(changeLive:)]){
    ...
    //点亮
        ...
        if([msgtype isEqual:@"0"]){
            ...
            //点亮
            if ([action isEqual:@"2"]) {
                if ([self.socketDelegate respondsToSelector:@selector(sendLight)]) {
    ...
    //设置管理员
        ...
        if([msgtype isEqual:@"4"] && [action isEqual:@"13"]) {
            ...
            if ([touid isEqual:[Config getOwnID]]) {
                [self alertShowMsg:minstr([msg valueForKey:@"ct"]) andTitle:@"提示"];
            }
        }
        ...
        if ([self.socketDelegate respondsToSelector:@selector(setSystemNot:)]) {
        ...
        if (ct) {
            if ([self.socketDelegate respondsToSelector:@selector(setAdmin:)]) {
...
//        NSDictionary *chat6 = [NSDictionary dictionaryWithObjectsAndKeys:uname,@"userName",ctt,@"contentChat",levell,@"levelI",ID,@"id",titleColor,@"titleColor",avatar,@"avatar",vip_type,@"vip_type",liangname,@"liangname",nil];
        if([self.socketDelegate respondsToSelector:@selector(sendGift:andLiansong:andTotalCoin:andGiftInfo:)]) {
    ...
    {
        if([self.socketDelegate respondsToSelector:@selector(SendBarrage:)]) {
    ...
    {
        if([self.socketDelegate respondsToSelector:@selector(StartEndLive)]) {
    ...
    {
        if([self.socketDelegate respondsToSelector:@selector(UserDisconnect:)]) {
    ...
    else if([method isEqual:@"KickUser"])
    {
        ...
        if([self.socketDelegate respondsToSelector:@selector(KickUser:)]) {
    ...
    ////收到主播广播准备开始游戏
        ...
        if ([action isEqual:@"1"]) {
            //出现游戏界面
            if([self.socketDelegate respondsToSelector:@selector(prepGameandMethod:andMsgtype:)]) {
            ...
            if ([method isEqual:@"startCattleGame"]) {
                ...
                if([self.socketDelegate respondsToSelector:@selector(changeBank:)]) {
                    [self.socketDelegate changeBank:bankdic];
                }
               
            }
            if([self.socketDelegate respondsToSelector:@selector(takePoker:Method:andMsgtype:)]) {
                //开始发牌
            ...
            if([self.socketDelegate respondsToSelector:@selector(stopGame)]) {
                //主播关闭游戏
...
#pragma mark 连麦
     ...
     */
        ...
        if ([action isEqual:@"5"]){
            ...
            if (![minstr([msg valueForKey:@"uid"]) isEqual:[Config getOwnID]]) {
                [MBProgressHUD showError:[NSString stringWithFormat:@"%@已下麦",minstr([msg valueForKey:@"uname"])]];
            }
        }
        if ([action isEqual:@"6"]) {
            [self.socketDelegate hostout];
            if ([minstr([msg valueForKey:@"touid"]) isEqual:[Config getOwnID]]) {
                [self alertShowMsg:nil andTitle:@"主播已把你下麦"];
            }else{
                [MBProgressHUD showError:[NSString stringWithFormat:@"%@已下麦",minstr([msg valueForKey:@"uname"])]];
            }
        }
        ...
        if ([touid isEqual:[Config getOwnID]]) {
            if ([action isEqual:@"2"]) {
                ...
                [self alertShowMsg:nil andTitle:@"主播拒绝了连麦请求"];
            ...
            if ([action isEqual:@"7"]) {
                NSString *touid = [NSString stringWithFormat:@"%@",[msg valueForKey:@"touid"]];
                if ([touid isEqual:[Config getOwnID]]) {
                    [self alertShowMsg:nil andTitle:@"主播正忙碌"];
            ...
            if ([action isEqual:@"8"]) {
                NSString *touid = [NSString stringWithFormat:@"%@",[msg valueForKey:@"touid"]];
                if ([touid isEqual:[Config getOwnID]]) {
                    [self alertShowMsg:nil andTitle:@"当前主播暂时无法接通"];
    ...
    //购买守护
        ...
        NSDictionary *dic = @{@"ct":[NSString stringWithFormat:@"%@ 守护了主播",minstr([msg valueForKey:@"uname"])]};
                             ...
                             @{
                                 @"msg":
                                     @[@{
                                           @"_method_":@"ShutUpUser",
                                           @"action":@"1",
                                           @"ct":[NSString stringWithFormat:@"%@被禁言%@",name,_shut_time],
                                           @"uid":[Config getOwnID],
                                           @"touid":ID,
