...
//            connectAlert = [[UIAlertView alloc]initWithTitle:@"提示" message:name delegate:self cancelButtonTitle:@"拒绝" otherButtonTitles:@"接受", nil];
//            [connectAlert show];
        }
        ...
        [self.delegate socketSystem:[NSString stringWithFormat:@"%@ 守护了主播",minstr([msg valueForKey:@"uname"])]];
        ...
        switch (action) {
            case 1:
                if (lianmaitime != 10) {
                    //主播正忙碌
                    [self anchor_operationAndAction:@"7"];
                    return;
                ...
                break;
            case 3:
                [MBProgressHUD showError:[NSString stringWithFormat:@"对方拒绝了你的连麦申请"]];
                isLianMai = NO;
                break;
            case 4:
            ...
            case 7:
                [MBProgressHUD showError:@"对方正忙碌"];
                isLianMai = NO;
                break;
            case 8:
                [MBProgressHUD showError:@"对方无响应"];
                isLianMai = NO;
    ...
    else if([method isEqual:@"SendRed"])
    {
        ...
        switch (action) {
                ...
                break;
                case 3:
                [MBProgressHUD showError:[NSString stringWithFormat:@"对方拒绝了你的PK申请"]];
...
//        linkAlert.timeL.text = [NSString stringWithFormat:@"发起PK请求(%ds)...",lianmaitime];
        ...
        linkAlert.timeL.text = [NSString stringWithFormat:@"发起连麦请求(%ds)...",lianmaitime];
    }
...
- (void)showLinkAlert:(NSDictionary *)dic{
    ...
    if (isPK) {
        ...
        linkAlert.timeL.text = @"发起连麦请求(10s)...";
    }
...
- (void)checkLinkLive:(NSDictionary *)dic{
    [YBToolClass postNetworkWithUrl:[NSString stringWithFormat:@"Livepk.CheckLive&stream=%@",minstr([dic valueForKey:@"stream"])] andParameter:nil success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        [lianmaitimer invalidate];
        lianmaitimer = nil;
        lianmaitime = 10;

        if (code == 0) {
            [self anchor_Agree:nil];
            isLianMai = YES;
            [self.delegate changeLivebroadcastLinkState:YES];
...
- (void)launchPK{
    if (!connectID) {
        [MBProgressHUD showError:@"不能发起PK"];
        return;
    }
    NSArray *msgData =@[
                        @{
...
- (void)showPKAlert:(NSDictionary *)dic{
    ...
    [[UIApplication sharedApplication].keyWindow addSubview:pkAlertView];
}
- (void)agreePKClick{
