...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    [AppInstanceTool sharedSingleton];
  ...
  //  [UMConfigure initWithAppkey:youmengKey channel:youmengChannel];
    ...
    self.tab =  [[ZYTabBarController alloc] init];
...
//        login.fd_prefersNavigationBarHidden = YES;
//        self.window.rootViewController = [[TCNavigationController alloc] initWithRootViewController:login];
//        [self.window makeKeyAndVisible];
//
//    }
...
////                [JMSGChatRoom enterChatRoomWithRoomId:[common jpush_sys_roomid] completionHandler:^(id resultObject, NSError *error) {
////                    if (!error) {
////                        NSLog(@"appdelegate-极光IM加入聊天室成功");
////                    }else{
////                        NSLog(@"appdelegate-极光IM加入聊天室失败\nerror=%@",error);
////                    }
////                }];
//            } else {
         ...
         if ([minstr([userInfo valueForKey:@"type"]) isEqual:@"1"]) {
             ...
             if([[userInfo valueForKey:@"userinfo"] valueForKey:@"uid"] != nil)
             {
                 if ([[NSUserDefaults standardUserDefaults] boolForKey:@"isLiveing"]) {
                     [MBProgressHUD showError:@"正在直播中，无法退出"];
                     return;
                     ...
                     UIAlertController *alertContro = [UIAlertController alertControllerWithTitle:@"提示" message:@"是否退出直播间"  preferredStyle:UIAlertControllerStyleAlert];
