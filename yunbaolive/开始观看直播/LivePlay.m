...
//    setFrontV.onlineLabel.text = [NSString stringWithFormat:@"%lld",userCount];
    ...
    if (playrtmp) {
        if ([[msg valueForKey:@"uid"] integerValue] == playrtmp.tag-1500) {
...
-(void)kickOK{
    UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"你已被踢出房间" message:nil delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
    [alert show];
    [self dissmissVC];
}
...
//    [pushBTN setTitle:@"发送" forState:UIControlStateNormal];
...
-(void)doUpMessageGuanZhu{
    if ([userView.forceBtn.titleLabel.text isEqual:@"已关注"]) {
        [userView.forceBtn setTitle:@"关注" forState:UIControlStateNormal];
       ...
       // NSLog(@"%@",error.errorDescription);
        [userView.forceBtn setTitle:@"已关注" forState:UIControlStateNormal];
        [userView.forceBtn setTitleColor:[UIColor lightGrayColor] forState:UIControlStateNormal];
...
-(void)pushMessage:(UITextField *)sender{
    if (keyField.text.length >50) {
        [MBProgressHUD showError:@"字数最多50字"];
        return;
    }
    pushBTN.enabled = NO;
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
...
-(void)lastview:(NSDictionary *)dic{
    ...
    NSArray *labelArray = @[@"直播时长",[NSString stringWithFormat:@"收获%@",[common name_votes]],@"观看人数"];
...
-(void)getNodeJSInfo
{
    ...
    [self.playDoc setValue:@"22001_1556599058" forKey:@"stream"];
     [self.playDoc setValue:@"22001" forKey:@"uid"];
...
-(void)KSVidioPlay
{
    ...
    _url = [NSURL URLWithString:@"rtmp://football1live.hengyunyouting.com/live/stream2300456"];
    [self setupObservers];
    NSLog(@"======播流地址%@",_url);
    [self onPlayVideo];
}
...
-(void)switchState:(BOOL)state
{
    ...
    else
    {
        ...
        keyField.placeholder = [NSString stringWithFormat:@"开启大喇叭，%@%@/条",self.danmuprice,[common name_coin]];
    }
}
    ...
    [session POST:url parameters:subdic progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if([number isEqualToNumber:[NSNumber numberWithInt:200]])
        {
            ...
            if([code isEqual:@"0"])
            {
                ...
                //修改按钮的颜色，同上可以使用同样的方法修改内容，样式
                UIAlertAction *cancelActionss = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
...
//                    secretAlert = [[UIAlertView alloc]initWithTitle:@"请填写密码" message:nil delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
...
//                    coastAlert = [[UIAlertView alloc]initWithTitle:type_msg message:nil delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
//                    [coastAlert show];
                }
...
- (void)playLinkUserUrl:(NSString *)playurl andUid:(NSString *)userid{
    [self releasePlayLinkView];
    playrtmp = [[play_linkMic alloc]initWithRTMPURL:@{@"playurl":playurl,@"pushurl":@"0"} andFrame:CGRectMake(_window_width - 100, _window_height - 100 -statusbarHeight - 150 -ShowDiff, 100, 150) andisHOST:NO];
    
    playrtmp.delegate = self;
    playrtmp.tag = 1500 + [userid intValue];
    [self.view addSubview:playrtmp];
...
-(void)connectVideos{
    ...
    if (!haslianmai) {
        if (playrtmp) {
            [MBProgressHUD showError:@"主播连麦中，请等会儿再试哦～"];
            return;
        }
    }
    if (haslianmai == NO) {
        haslianmai = YES;
        //发送连麦socket
        [MBProgressHUD showError:@"连麦请求已发送"];
        ...
        if (startLinkTimer) {
            [MBProgressHUD showError:@"您已申请，请稍等"];
            return;
        }
...
-(void)startConnectvideo{
    ...
    NSString *url = [purl stringByAppendingFormat:@"?service=Linkmic.RequestLVBAddrForLinkMic&uid=%@",[Config getOwnID]];
    NSString *newUrlStr = [url stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    AFHTTPSessionManager *session = [AFHTTPSessionManager manager];
    [session POST:newUrlStr parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSNumber *number = [responseObject valueForKey:@"ret"];
        if([number isEqualToNumber:[NSNumber numberWithInt:200]])
        {
            NSArray *data = [responseObject valueForKey:@"data"];
            ...
            {
                NSDictionary *info = [[data valueForKey:@"info"] firstObject];
                _myplayurl = [info valueForKey:@"playurl"];
                NSString *_mypushurl = [info valueForKey:@"pushurl"];
                NSDictionary *subdic = @{
                ...
                [self releasePlayLinkView];
                playrtmp = [[play_linkMic alloc]initWithRTMPURL:subdic andFrame:CGRectMake(_window_width - 100, _window_height - 110 - ShowDiff - 150 , 100, 150) andisHOST:NO];
                
                playrtmp.delegate = self;
                playrtmp.tag = 1500 + [[Config getOwnID] intValue];
                [self.view addSubview:playrtmp];
                [self.view insertSubview:playrtmp aboveSubview:self.tableView];
...
- (void)releasePlayLinkView{
    if (playrtmp) {
        [playrtmp stopConnect];
        [playrtmp stopPush];
        [playrtmp removeFromSuperview];
        playrtmp = nil;
    }
    if (startLinkTimer) {
        [startLinkTimer invalidate];
...
- (void)anchor_agreeLink:(NSDictionary *)dic{
    ...
    playrtmp = [[play_linkMic alloc]initWithRTMPURL:@{@"playurl":minstr([dic valueForKey:@"pkpull"]),@"pushurl":@"0",@"userid":minstr([dic valueForKey:@"pkuid"])} andFrame:CGRectMake(_window_width/2, 130+statusbarHeight , _window_width/2, _window_width*2/3) andisHOST:NO];
    playrtmp.delegate = self;
...
- (void)anchor_stopLink:(NSDictionary *)dic{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        _player.view.frame = CGRectMake(0, 0, _window_width, _window_height);
    }];
}
#pragma mark ================ PK ===============
- (void)showPKView:(NSDictionary *)dic{
    ...
    pkView = [[anchorPKView alloc]initWithFrame:CGRectMake(0, 130+statusbarHeight, _window_width, _window_width*2/3+20) andTime:time];
    pkView.delegate = self;
    [videoView addSubview:pkView];
}
